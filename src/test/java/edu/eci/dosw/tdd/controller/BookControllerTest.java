...
class BookControllerTest {
    ...
    void createBook_emptyTitle_returns400() throws Exception {
        BookDTO bad = new BookDTO(null, "", "Autor", 1, 0);
        mockMvc.perform(post("/api/books")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("POST /api/books - autor vacío retorna 400")
    void createBook_emptyAuthor_returns400() throws Exception {
        BookDTO bad = new BookDTO(null, "Título", "", 1, 0);
        mockMvc.perform(post("/api/books")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(bad)))
                .andExpect(status().isBadRequest());
    ...
    void updateBook_valid_returns200() throws Exception {
        ...
        BookDTO update = new BookDTO(null, "Título Actualizado", "Nuevo Autor", 5, 0);
        mockMvc.perform(put("/api/books/" + id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(update)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.title", is("Título Actualizado")))
                .andExpect(jsonPath("$.author", is("Nuevo Autor")));
    }

    @Test
    @DisplayName("PUT /api/books/{id} - ID inexistente retorna 404")
    void updateBook_notFound_returns404() throws Exception {
        BookDTO update = new BookDTO(null, "Título", "Autor", 1, 0);
        mockMvc.perform(put("/api/books/INVALID")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(update)))
                .andExpect(status().isNotFound());
    }
    ...
    @Test
    @DisplayName("GET /api/books/search - sin coincidencias retorna lista vacía")
    void searchByTitle_noMatch_returnsEmpty() throws Exception {
        postBook(validBook());
        mockMvc.perform(get("/api/books/search").param("title", "xyz"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(0)));
    }
}